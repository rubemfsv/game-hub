...
export class ResourceManager {
  ...
  public async loadAssets(): Promise<{
    ...
    const { assetsToLoad: emojiAssets, emojiUrlMap } = this.prepareAssetUrls(allEmojiNames);

    const allCharacterNames = this.getAllCharacterNames();
    const { assetsToLoad: avatarAssets, avatarUrlMap } = this.prepareAvatarUrls(allCharacterNames);

    await PIXI.Assets.load([...emojiAssets, ...avatarAssets]);

    const emojiTextures = this.createEmojiTextures(allEmojiNames, emojiUrlMap);
    const avatarSprites = this.createAvatarSprites(allCharacterNames, avatarUrlMap);
  ...
  private prepareAssetUrls(allEmojiNames: Set<string>): {
    ...
    allEmojiNames.forEach(name => {
      const url = apiEmojis.get(name) || `https://api.dicebear.com/9.x/fun-emoji/png?seed=${name}`;
  ...
  private prepareAvatarUrls(characterNames: Set<string>): { assetsToLoad: { alias: string; src: string }[]; avatarUrlMap: Map<string, string> } {
    const assetsToLoad: { alias: string; src: string }[] = [];
    ...
    characterNames.forEach(name => {
      ...
      const url = existingAvatar?.url ?? `${fallbackUrl}?seed=${encodeURIComponent(name)}`;
  ...
  private createAvatarSprites(characterNames: Set<string>, avatarUrlMap: Map<string, string>): Record<string, PIXI.Sprite> {
